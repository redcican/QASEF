"""Experiment: Incomplete data setting (Table 3 in the paper).

Evaluates QASEF under varying missing rates rho in {10%, 30%, 50%, 70%}.
"""
...
from qasef.utils import generate_missing_mask
...
def run_incomplete(dataset_name: str, data_dir: str | None = None):
    ...
    for rho in MISSING_RATES:
        print(f"\n  Missing rate: {rho*100:.0f}%")
        all_results = []
        for run in range(N_RUNS):
            ...
            pred = model.fit_predict(views, M)
